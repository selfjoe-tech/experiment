...
import { fetchVideoForEmbed } from "@/lib/actions/embed";
import EmbedPlayerClient from "./player-client";
...
export default async function EmbedVideoPage({
  ...
  const video: Video | null = await fetchVideoForEmbed(mediaId);
  if (!video) notFound();

  // Server output includes real markup; client adds your interactions.
  return <EmbedPlayerClient initialVideo={video} />;
}
