...
function Badge({ tone, children }: { tone: "neutral" | "good" | "bad" | "warn"; children: React.ReactNode }) {
  const cls =
    tone === "good"
...
export default function AffiliateRequestsClient({
  initial,
  filter,
}: {
  initial: { rows: AffiliateRequestRow[]; total: number; page: number; pageSize: number };
  ...
  const onAccept = (id: string) => {
    startTransition(async () => {
      const res = await adminAcceptAffiliateRequest(id);
      if (!res.ok) return;
      router.refresh();
    });
  };
  ...
  const onReject = (id: string) => {
    startTransition(async () => {
      const res = await adminRejectAffiliateRequest(id);
      if (!res.ok) return;
      router.refresh();
    });
  };
  ...
  return (
    <div className="space-y-4">
              ...
              initial.rows.map((r, idx) => {
                ...
                return (
                  <tr key={r.id} className={`border-b border-white/5 ${zebra}`}>
                    ...
                    <td className="py-3 px-4 text-white/70">{fmtDate(r.created_at)}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => onAccept(r.id)}
                          disabled={pending || r.status === "accepted"}
                          className="h-9 rounded-xl bg-emerald-400/90 text-black px-3 font-semibold disabled:opacity-60"
                        >
                          Accept
                        </button>
                        <button
                          ...
                          className="h-9 rounded-xl border border-white/15 bg-black/30 text-white px-3 font-semibold disabled:opacity-60"
                        >
                          Reject
                        </button>
                      </div>
      ...
      <div className="md:hidden space-y-3">
        {initial.rows.length === 0 ? (
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-center text-white/60">No requests found.</div>
        ) : (
          initial.rows.map((r) => {
            const statusTone = r.status === "accepted" ? "good" : r.status === "rejected" ? "bad" : "warn";
            return (
              <div key={r.id} className="rounded-3xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-white/60">{fmtDate(r.created_at)}</div>
                  </div>
                  <Badge tone={statusTone}>{r.status}</Badge>
                </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                    onClick={() => onAccept(r.id)}
                    disabled={pending || r.status === "accepted"}
                    className="h-11 rounded-2xl bg-emerald-400/90 text-black font-semibold disabled:opacity-60 inline-flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    onClick={() => onReject(r.id)}
                    disabled={pending || r.status === "rejected"}
                    className="h-11 rounded-2xl border border-white/15 bg-black/30 text-white font-semibold disabled:opacity-60 inline-flex items-center justify-center gap-2"
                  >
